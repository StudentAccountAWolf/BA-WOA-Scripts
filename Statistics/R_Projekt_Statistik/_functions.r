...
# ---------- PLS ----------
f.select_features_pls <- function(feat_matrix, sel_factor, sel_colors, components=2, tune_length=10, quantile_threshold=0.95, keepx_min=10, keepx_max=0, plot_roc_filename=NULL) {
...
# ---------- sparse PLS ----------
f.select_features_spls <- function(feat_matrix, sel_factor, sel_colors, components=2, eta=0.7, kappa=0.5, tune_length=10, quantile_threshold=0.95, plot_roc_filename=NULL) {
	# Make factors readible by R
	sel_factor <- as.factor(make.names(sel_factor))
	...
	# Train spls model
	model_spls <- caret::train(x=as.matrix(feat_matrix), y=sel_factor, method="spls",
							   preProcess=c("center", "scale"), probMethod="softmax",
							   tuneGrid=data.frame(K=components, eta=eta, kappa=kappa),
							   tuneLength=tune_length, trControl=caret::trainControl(method="repeatedcv", number=10, repeats=5, classProbs=TRUE, savePredictions="final"))
	#print(paste("Number of chosen components:",as.numeric(model_spls$bestTune)))
	
	# Get variable importances
	imp_spls <- varImp(object=model_spls)
	rownames(imp_spls$importance) <- as.character(rownames(imp_spls$importance))
	imp_spls$importance <- imp_spls$importance/100
	imp_spls$importance[is.na(imp_spls$importance)] <- 0
	
	# Names of selected features
	sel_spls <- f.select_features_from_model(feat_list=feat_matrix, feat_class=sel_factor, model_varimp=imp_spls$importance, confidence=quantile_threshold, keepx_min=10)
	
	# Save selected variables
	sel_spls[["_selected_variables_"]] <- unique(unlist(f.select_features_from_model(feat_list=feat_matrix, feat_class=sel_factor, model_varimp=imp_spls$importance, confidence=quantile_threshold, keepx_min=10)))
	sel_spls[["_varImp_"]] <- as.data.frame(imp_spls$importance)
	
	# Performance measures
	sel_spls <- do.call(c, list(sel_spls, f.performance_measures_caret(model=model_spls, sel_factor=sel_factor, sel_colors=sel_colors)))
...
# ---------- sPLS ----------
f.select_features_spls_mixomics <- function(feat_matrix, sel_factor, sel_colors, mode="classic", tune_components=2, sel_components=c(1,2), folds_number=10, keepx=c(100,50,25), plot=FALSE, plot_roc_filename=NULL) {
	# Make factors readible by R
	sel_factor <- as.factor(make.names(sel_factor))
...
# ---------- sPLS-DA ----------
f.select_features_splsda_mixomics <- function(feat_matrix, sel_factor, sel_colors, tune_components=2, sel_components=c(1,2), folds_number=10, keepx=c(100,50,25), plot=FALSE, plot_roc_filename=NULL) {
	# Make factors readible by R
	sel_factor <- as.factor(make.names(sel_factor))
...
# ---------- Random Forest ----------
f.select_features_random_forest <- function(feat_matrix, sel_factor, sel_colors, tune_length, quantile_threshold, keepx_min=10, keepx_max=0, plot_roc_filename=NULL) {
	# Make factors readible by R
	sel_factor <- as.factor(make.names(sel_factor))
...
# ---------- Bayes inference ----------
f.select_features_bayes <- function(feat_matrix, sel_factor, sel_colors, tune_length, quantile_threshold, plot_roc_filename=NULL) {
	# Load library
	library(naivebayes)
	...
	model_bayes <- caret::train(x=as.matrix(feat_matrix), y=sel_factor, method="naive_bayes", importance=TRUE, proximity=TRUE,
								tuneLength=tune_length, trControl=caret::trainControl(method="repeatedcv", number=10, repeats=5, classProbs=TRUE, savePredictions="final"))
	
	# Get variable importances
	imp_bayes <- varImp(object=model_bayes)
	rownames(imp_bayes$importance) <- as.character(rownames(imp_bayes$importance))
	imp_bayes$importance <- imp_bayes$importance / 100
	
	# Save names of selected features
	sel_bayes <- f.select_features_from_model(feat_list=feat_matrix, feat_class=sel_factor, model_varimp=imp_bayes$importance, confidence=quantile_threshold, keepx_min=10)
	
	# Save selected variables
	sel_bayes[["_selected_variables_"]] <- unique(unlist(f.select_features_from_model(feat_list=feat_matrix, feat_class=sel_factor, model_varimp=imp_bayes$importance, confidence=quantile_threshold, keepx_min=10)))
	sel_bayes[["_varImp_"]] <- as.data.frame(imp_bayes$importance)
	
	# Performance measures
	sel_bayes <- do.call(c, list(sel_bayes, f.performance_measures_caret(model=model_bayes, sel_factor=sel_factor, sel_colors=sel_colors)))
...
# ---------- Tree augmented Naive Bayes inference ----------
f.select_features_bayes_tree <- function(feat_matrix, sel_factor, sel_colors, tune_length, quantile_threshold, plot_roc_filename=NULL) {
	# Load library
	library(bnclassify)
	...
	# Train Naive Bayes model
	model_bayes <- caret::train(x=as.matrix(feat_matrix), y=sel_factor, method="tan",
								importance=TRUE, proximity=TRUE,
								tuneGrid=data.frame(score="aic", smooth=0),
								tuneLength=tune_length, trControl=caret::trainControl(method="repeatedcv", number=10, repeats=5, classProbs=TRUE, savePredictions="final"))
...
# ---------- Adaboost ----------
f.select_features_adaboost <- function(feat_matrix, sel_factor, sel_colors, tune_length, quantile_threshold, plot_roc_filename=NULL) {
	# Load library
	library(adabag)
	library(rpart)
	library(doParallel)
	...
	# Train Naive adaboost model
	model_adaboost <- caret::train(x=as.matrix(feat_matrix), y=sel_factor, method="AdaBoost.M1", importance=TRUE, proximity=TRUE,
								   tuneLength=tune_length, trControl=caret::trainControl(method="repeatedcv", number=10, repeats=5, classProbs=TRUE, savePredictions="final"))
	
	# Get variable importances
	imp_adaboost <- varImp(object=model_adaboost)
	rownames(imp_adaboost$importance) <- as.character(rownames(imp_adaboost$importance))
	
	# Save names of selected features
	sel_adaboost <- f.select_features_from_model(feat_list=feat_matrix, feat_class=sel_factor, model_varimp=imp_adaboost$importance, confidence=quantile_threshold, keepx_min=10)
	
	# Save selected variables
	sel_adaboost[["_selected_variables_"]] <- unique(unlist(f.select_features_from_model(feat_list=feat_matrix, feat_class=sel_factor, model_varimp=imp_adaboost$importance, confidence=quantile_threshold, keepx_min=10)))
	sel_adaboost[["_varImp_"]] <- as.data.frame(imp_adaboost$importance)
	
	# Performance measures
	sel_adaboost <- do.call(c, list(sel_adaboost, f.performance_measures_caret(model=model_adaboost, sel_factor=sel_factor, sel_colors=sel_colors)))
...
f.select_features_svm <- function(feat_matrix, sel_factor, sel_colors, tune_length, quantile_threshold, plot_roc_filename=NULL) {
	# Load library
	library(e1071)
	...
	# Train Naive SVM model
	model_svm <- caret::train(x=as.matrix(feat_matrix), y=sel_factor, method="svmLinear2", importance=TRUE, proximity=TRUE,
							  preProcess=c("center", "scale"),
							  tuneGrid=data.frame(cost=c(1), kernel="radial", probability=c(TRUE), tolerance=c(0.001), epsilon=c(0.1)),
							  tuneLength=tune_length, trControl=caret::trainControl(method="repeatedcv", number=10, repeats=5, classProbs=TRUE, savePredictions="final"))
	
	# Get variable importances
	imp_svm <- varImp(object=model_svm)
	rownames(imp_svm$importance) <- as.character(rownames(imp_svm$importance))
	
	# Save names of selected features
	sel_svm <- f.select_features_from_model(feat_list=feat_matrix, feat_class=sel_factor, model_varimp=imp_svm$importance, confidence=quantile_threshold, keepx_min=10)
	
	# Save selected variables
	sel_svm[["_selected_variables_"]] <- unique(unlist(f.select_features_from_model(feat_list=feat_matrix, feat_class=sel_factor, model_varimp=imp_svm$importance, confidence=quantile_threshold, keepx_min=10)))
	sel_svm[["_varImp_"]] <- as.data.frame(imp_svm$importance)
...
f.select_features_random_forest_boruta <- function(feat_matrix, sel_factor, sel_colors, sel_method="best", tune_length=10, pvalue=0.05, mcAdj=TRUE, maxRuns=1000, plot_roc_filename=NULL) {
	...
	sel_boruta <- foreach(i=c(1:tune_length)) %dopar% {
		...
		#sel_boruta[[(length(sel_boruta)+1)]] <- sel_boruta_rf
		return(sel_boruta_rf)
	}
	
	# Selected variables
	if (sel_method=="best") {
		sel_var <- sel_boruta[[1]]$`_selected_variables_`
		for (i in 2:length(sel_boruta)) {
			sel_var <- sel_var[sel_var %in% sel_boruta[[i]]$`_selected_variables_`]
		}
	} else {
		sel_var <- sel_boruta[[1]]$`_selected_variables_`
		for (i in 2:length(sel_boruta)) {
			sel_var <- c(sel_var, sel_boruta[[i]]$`_selected_variables_`)
		}
		sel_var <- unique(sel_var)
	}
	
	# Train RF model with selected BORUTA model
	model_rf <- caret::train(x=as.matrix(feat_matrix[, sel_var]), y=sel_factor, method="rf", importance=TRUE, proximity=TRUE,
							 tuneLength=tune_length, trControl=caret::trainControl(method="repeatedcv", number=10, repeats=5, classProbs=TRUE, savePredictions="final"))
	...
	# Save selected variables
	sel_boruta[["_selected_variables_"]] <- sel_var
	sel_boruta[["_varImp_"]] <- as.data.frame(imp_rf$importance)
	...
	# Performance measures
	sel_boruta <- do.call(c, list(sel_boruta, f.performance_measures_caret(model=model_rf, sel_factor=sel_factor, sel_colors=sel_colors)))
...
# ---------- Recursive Partitioning and Regression Trees in CARET ----------
f.select_features_rpart_caret <- function(feat_matrix, sel_factor, sel_colors, tune_length=100, keepx_min=1, keepx_max=0, quantile_threshold=0.95, plot_roc_filename=NULL) {
	# Make factors readible by R
	sel_factor <- as.factor(make.names(sel_factor))
	...
	# Train PLS-DA model
	model_rpart <- caret::train(x=as.matrix(feat_matrix), y=sel_factor, method="rpart",
								tuneLength=tune_length, trControl=caret::trainControl(method="repeatedcv", number=10, repeats=5, classProbs=TRUE, savePredictions="final"))
	
	# Get variable importances
	imp_rpart <- varImp(object=model_rpart)
	rownames(imp_rpart$importance) <- as.character(rownames(imp_rpart$importance))
	
	# Save variable importances
	sel_rpart <- list()
	sel_rpart[["_varImp_"]] <- as.data.frame(imp_rpart$importance)
	
	# Save best model
	sel_rpart[["_finalModel_"]] <- model_rpart$finalModel
	
	# Performance measures
	sel_rpart <- do.call(c, list(sel_rpart, f.performance_measures_caret(model=model_rpart, sel_factor=sel_factor, sel_colors=sel_colors)))
...
# ---------- Recursive Partitioning and Regression Trees in CARET ----------
f.select_features_ctree_caret <- function(feat_matrix, sel_factor, sel_colors, tune_length=100, keepx_min=1, keepx_max=0, quantile_threshold=0.95, plot_roc_filename=NULL) {
	# Make factors readible by R
	sel_factor <- as.factor(make.names(sel_factor))
	...
	# Train PLS-DA model
	model_ctree <- caret::train(x=as.matrix(feat_matrix), y=sel_factor, method="ctree",
								tuneLength=tune_length, trControl=caret::trainControl(method="repeatedcv", number=10, repeats=5, classProbs=TRUE, savePredictions="final"))
	
	# Get variable importances
	imp_ctree <- varImp(object=model_ctree)
	rownames(imp_ctree$importance) <- as.character(rownames(imp_ctree$importance))
	
	# Save names of selected features
	sel_ctree <- f.select_features_from_model(feat_list=feat_matrix, feat_class=sel_factor, model_varimp=imp_ctree$importance, confidence=quantile_threshold, keepx_min=keepx_min, keepx_max=keepx_max)
	
	# Save selected variables
	sel_ctree[["_selected_variables_"]] <- unique(unlist(f.select_features_from_model(feat_list=feat_matrix, feat_class=sel_factor, model_varimp=imp_ctree$importance, confidence=quantile_threshold, keepx_min=keepx_min, keepx_max=keepx_max)))
	
	# Save variable importances
	sel_ctree[["_varImp_"]] <- as.data.frame(imp_ctree$importance)
	
	# Save best model
	sel_ctree[["_finalModel_"]] <- model_ctree$finalModel
	
	# Performance measures
	sel_ctree <- do.call(c, list(sel_ctree, f.performance_measures_caret(model=model_ctree, sel_factor=sel_factor, sel_colors=sel_colors)))
...
f.select_features_from_model <- function(feat_list, feat_class, model_varimp, keepx_min=10, keepx_max=0, confidence=0.95) {
	...
	for (i in unique(feat_class)) {
		...
		sel_list[[i]] <- colnames(feat_list)[base::order(model_varimp[, i], decreasing=TRUE)[1:select_elements]]
...
# ---------- Draw heatmap of selected features ----------
f.heatmap.selected_features <- function(feat_list, sel_feat, row_names=NULL, sel_names=NULL, sample_colors=NULL, filename, main, scale="col", plot_width=6, plot_height=5, cex_col=0.5, cex_row=0.7, rowsidecolors=NULL) {
	...
	if (is.null(rowsidecolors)) {
		...
		heatmap.2(x=as.matrix(sel_list), scale=scale, cexRow=cex_row, cexCol=cex_col, main=main,
				  ...
				  ###col=colorRampPalette(c('white','#ddffaa','forestgreen'), alpha=0.1, bias=1)(256),
...
f.make_classes_at_level <- function(level=2, div_samples) {
...
# Convert continuous quantitative traits to distinct character states
# Apply gap weighting algorithm by Thiele (1993), https://doi.org/10.1006/clad.1993.1020
gap_weighting <- function(morph_char, states=3) {
